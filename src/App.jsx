        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 }}>
          {items.map((it, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: 6, background: '#F1F3F1', flexShrink: 0, overflow: 'hidden' }}>
                {it.products?.image_url && (
                  <img src={it.products.image_url} alt={it.products?.name || ''} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
              </div>
              <span style={{ fontSize: 12, color: '#2C2C2A', flex: 1 }}>{it.products?.name || 'Article'}</span>
              <span style={{ fontSize: 12, color: COLORS.textMuted, fontWeight: 600 }}>×{it.quantity}</span>
            </div>
          ))}
        </div>
      )}
